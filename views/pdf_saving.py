...
async def data_preprocessing(task_id, pdf_path, skip_tags = None, category = None):
    ...
    sections = await execute_task('Detect Sections with Content', detect_section_with_content, pdf_path, skip_tags=skip_tags, category=category, headers=headers, footers=footers)
...
@app.post("/upload-pdf")
async def upload_pdf(backgroud_tasks: BackgroundTasks, pdf_file: UploadFile = File(...), skip_tags: List[str] = Form([]), category: str = Form([])):
    ...
    backgroud_tasks.add_task(data_preprocessing, task_id, temp_pdf_path, skip_tags, category)
